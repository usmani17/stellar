...
import { GoogleCampaignsTable, type GoogleCampaign } from "./components/GoogleCampaignsTable";
...
export const GoogleCampaigns: React.FC = () => {
  ...
  const [analyticsSyncMessage, setAnalyticsSyncMessage] = useState<string | null>(null);
  ...
  const loadCampaignsWithFilters = async (accountId: number, filterList: FilterValues) => {
    try {
      ...
      if (responseWithChart.chart_data && Array.isArray(responseWithChart.chart_data)) {
  ...
  const loadCampaigns = async (accountId: number) => {
    try {
      ...
      if (response.campaigns && response.campaigns.length > 0) {
        console.log("First campaign:", JSON.stringify(response.campaigns[0], null, 2));
        console.log("First campaign id:", response.campaigns[0].id);
        console.log("First campaign campaign_id:", response.campaigns[0].campaign_id);
      }
      const campaignsArray = Array.isArray(response.campaigns) ? response.campaigns : [];
      ...
      if (responseWithChart.chart_data && Array.isArray(responseWithChart.chart_data)) {
        console.log("✅ [CHART DEBUG] Setting chart data, length:", responseWithChart.chart_data.length);
        setChartDataFromApi(responseWithChart.chart_data);
      } else {
        console.log("❌ [CHART DEBUG] No chart_data found or not an array, setting empty array");
        setChartDataFromApi([]);
      }
  ...
  const handleSyncAnalytics = async () => {
    ...
    try {
      ...
      let message = result.message || `Successfully synced analytics: ${result.rows_inserted || 0} inserted, ${result.rows_updated || 0} updated`;
      ...
      const errorMessage = error.response?.data?.error || error.message || "Failed to sync campaign analytics from Google Ads";
  ...
  const chartData = useMemo(() => {
    console.log("📊 [CHART DEBUG] Processing chart data, chartDataFromApi length:", chartDataFromApi.length);
    ...
    if (chartDataFromApi.length > 0) {
      ...
      console.log("✅ [CHART DEBUG] Processed chart data, length:", processed.length, "first item:", processed[0]);
      return processed;
    }

    // Return empty array if no data from backend
    console.log("❌ [CHART DEBUG] No chart data from API, returning empty array");
    return [];
  }, [chartDataFromApi]);
        ...
        <div className="px-4 py-6 sm:px-6 lg:p-8 bg-white overflow-x-hidden min-w-0">
          <div className="space-y-6">
            ...
            {analyticsSyncMessage && (
              <div className="mb-4">
                <Banner
                  type={analyticsSyncMessage.includes("error") || analyticsSyncMessage.includes("Failed") ? "error" : "success"}
