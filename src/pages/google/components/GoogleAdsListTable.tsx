...
export const GoogleAdsListTable: React.FC<GoogleAdsListTableProps> = ({
  ...
  const isDraftAd = (row: GoogleAd) => {
    ...
    return (
      s === "SAVED_DRAFT" ||
      s === "DRAFT" ||
      String(row.ad_id ?? row.id).startsWith("draft-")
    );
  };
    ...
    () => [
      ...
      },
      {
        ...
        navigateTo: (row: GoogleAd, accountId: string) =>
          row.adgroup_id
            ? channelId
              ? `/brands/${accountId}/${channelId}/google/adgroups?adgroup_id=${row.adgroup_id}`
              : `/brands/${accountId}/google-adgroups?adgroup_id=${row.adgroup_id}`
            : null,
      ...
      },
      {
        ...
        navigateTo: (row: GoogleAd, accountId: string) =>
          row.campaign_id
            ? channelId
              ? `/brands/${accountId}/${channelId}/google/campaigns/${row.campaign_id}`
      ...
      },
      {
        ...
        getValue: (row: GoogleAd) =>
          (row as any).avg_cpc || (row as any).cpc || 0,
      },
      {
        key: "avg_cost",
        label: "Avg. cost",
        ...
        getValue: (row: GoogleAd) => {
          ...
          const interactions =
            (row as any).interactions || (row as any).clicks || 0;
          const spends = (row as any).spends || 0;
          return interactions > 0
            ? spends / interactions
            : (row as any).avg_cost || 0;
        },
      },
      {
