...
export const AdGroupsTab: React.FC<AdGroupsTabProps> = ({
  ...
  const handleStatusClick = (adgroup: GoogleAdGroup) => {
    if (onUpdateAdGroupStatus) {
      ...
      setEditingStatus(adgroup.status || "ENABLED");
    }
  };

  const handleStatusChange = async (adgroupId: number, newStatus: string) => {
    if (onUpdateAdGroupStatus && newStatus !== editingStatus) {
      try {
        await onUpdateAdGroupStatus(adgroupId, newStatus);
                ...
                {adgroups.map((adgroup, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-[10px] px-[10px]">
                        {editingAdGroupId === adgroup.id && onUpdateAdGroupStatus ? (
                          <Dropdown
                            options={[
                            ...
                            value={editingStatus}
                            onChange={(val) => handleStatusChange(adgroup.id, val as string)}
                            defaultOpen={true}
                            closeOnSelect={true}
                          ...
                          <div
                            className={onUpdateAdGroupStatus ? "cursor-pointer hover:underline" : ""}
