...
/**
 * Meta ad detail page (placeholder).
 * Route: /brands/:accountId/:channelId/meta/ads/:adId
 */
export function MetaAdDetail() {
  const { accountId, channelId, adId } = useParams<{
    accountId: string;
    channelId: string;
    adId: string;
  }>();
  ...
  useEffect(() => {
    setPageTitle("Meta Ad");
    return () => resetPageTitle();
  }, []);

  const listPath = `/brands/${accountId}/${channelId}/meta/ads`;
        ...
        <main className="flex-1 p-6">
          ...
          <div className="rounded-lg border border-[#e8e8e3] bg-white p-6">
            ...
            <p className="text-[#556179]">
              Ad ID: <code className="text-[#072929]">{adId}</code>
            </p>
