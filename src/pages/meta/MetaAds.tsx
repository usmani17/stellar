...
export const MetaAds: React.FC = () => {
  ...
  const handleSelectAll = () => {
    if (allSelected) {
      ...
      setSelectedAds(new Set(ads.map((a) => String(a.ad_id ?? a.id))));
    }
  };
                ...
                >
                  <div className="overflow-x-auto w-full">
                    <table className="min-w-[1200px] w-full">
                      ...
                      <tbody>
                        {ads.map((row) => {
                          const aid = String(row.ad_id ?? row.id);
                          return (
                            <tr
                              key={row.id ?? row.ad_id}
                            ...
                            >
                              <td className="table-cell sticky left-0 z-[120] bg-[#f5f5f0] group-hover:bg-gray-100 border-r border-[#e8e8e3] py-3 px-4 w-[35px]">
                                <Checkbox
                                  checked={selectedAds.has(aid)}
                                  onChange={() => handleSelectAd(aid)}
                                  aria-label={`Select ${row.ad_name || aid}`}
                                />
                              </td>
