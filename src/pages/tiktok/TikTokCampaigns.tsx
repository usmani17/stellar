  ...
  const handleCreateCampaign = async (data: CreateTikTokCampaignData) => {
    ...
    try {
      if (campaignFormMode === "edit" && editingCampaign) {
        ...
        // Create campaign
        const response = await campaignsService.createTikTokCampaign(
          parseInt(accountId),
          data
        );
      ...
      for (const filter of filters) {
        ...
        if (filter.field === "state") {
          ...
          const filterValues = Array.isArray(filter.value)
            ? filter.value
            : [filter.value];
          const mappedValues = filterValues.map(
            (v) => stateMapping[v as string] || v
          );
    ...
    try {
      ...
      if (inlineEditField === "operation_status") {
        await campaignsService.updateTikTokCampaignStatus(accountIdNum, {
          ...
          operation_status: inlineEditNewValue as
            | "ENABLE"
            | "DISABLE"
            | "DELETE",
        });
      ...
      for (const filter of filters) {
        ...
        if (filter.field === "state") {
          ...
          const matchesFilter = filterValues.some((filterVal) => {
            const validStatuses = statusMapping[filterVal as string] || [
              filterVal,
            ];
      ...
      >
        ...
        <div className="px-4 py-6 sm:px-6 lg:p-8 bg-white overflow-x-hidden min-w-0">
          <div className="space-y-6">
                  ...
                  initialData={
                    editingCampaign
                      ? {
                          ...
                          budget_optimize_on: (editingCampaign as any)
                            .budget_optimize_on,
            ...
            {/* Search, Edit, Export with overlay when panel is open */}
            <div className="relative">
        ...
        >
          <div className="bg-white rounded-xl shadow-lg max-w-4xl w-full mx-4 p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-sandstorm-s10 border border-sandstorm-s40 rounded-lg p-4 mb-4">
              <div className="flex items-center gap-2">
                <span className="text-[12.16px] text-[#556179]">
                  ...
                  {selectedCampaigns.size !== 1 ? "s" : ""} will be updated:
                </span>
            ...
            {(() => {
              ...
              const previewCount = Math.min(10, selectedCampaignsData.length);
                ...
                <div className="mb-6">
                  ...
                  <div className="border border-gray-200 rounded-lg overflow-hidden">
                    <table className="w-full">
                      ...
                      <tbody>
                        {selectedCampaignsData.slice(0, 10).map((campaign) => {
                          const oldStatus =
            ...
            <div className="space-y-3 mb-6">
              <div className="flex justify-between items-center py-2 border-b border-gray-200">
                ...
                <span className="text-[12.16px] font-semibold text-[#072929]">
                  {pendingStatusAction === "ENABLE" ? "Enable" : "Pause"}
                </span>
              </div>
            </div>
        ...
        >
          <div className="bg-white rounded-xl shadow-lg max-w-md w-full mx-4 p-6">
            ...
            <p className="text-[12.16px] text-[#556179] mb-4">
              You are about to permanently delete {selectedCampaigns.size}{" "}
              selected campaign
              {selectedCampaigns.size !== 1 ? "s" : ""}. This will stop all ad
              serving immediately and cannot be undone. Deleted campaigns can
              still be viewed in reports but not edited or re-enabled.
            </p>
      ...
      {showInlineEditModal && inlineEditCampaign && inlineEditField && (
        ...
        >
          <div className="bg-white rounded-xl shadow-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-[17.1px] font-semibold text-[#072929] mb-4">
              Confirm{" "}
              {inlineEditField === "operation_status" ? "Status" : "Budget"}{" "}
              Change
            </h3>
            <div className="mb-4">
